<template>
  <DefaultLayout>
    <form @submit="editarEvento">
      <div class="container d-flex flex-column mt-5" v-if="!loadingAsistentes">
        <h2 class="text-center mb-4 center-content">Eventos - Editar</h2>
      ...
      <div class="d-flex justify-content-center  mb-5 mt-3">
        ...
        <button type="submit" class="btn btn-success" :disabled="loading">
          ...
          <i class="bi bi-plus me-1" :class="{ 'd-none': loading }"></i>Editar Evento
        </button>
      </div>
    </form>

    <div class="alert alert-success align-self-center" v-if="mensajeExito">Evento actualizado exitosamente.
...
export default {
    ...
    await eventoStore.detalleEvento(this.$route.params.id)
      ...
      .then(data => {
        if (data.status == 401) {
          ...
          this.evento.nombre = data.data.nombre;
          this.evento.lugar = data.data.lugar;
          this.evento.fecha = data.data.fecha;
          this.evento.descripcion = data.data.descripcion;
          let asistentes = data.data.asistentes.map(asistente => asistente.id)
          this.evento.asistentes = asistentes;
  ...
  methods: {
    editarEvento: async function (e) {
      ...
      this.loading = true;
      await this.eventoStore.editarEvento(this.$route.params.id, this.evento)
        .catch(e => console.error(e))
        .then(data => {
          this.loading = false;
          if (data.status == 200) {
