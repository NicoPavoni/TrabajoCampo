<template>
    <DefaultLayout>
        ...
        <h2 class="text-center mt-3">Listado de Eventos</h2>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>¿Estas seguro que desea eliminar el evento '{{ modalEliminar.nombre }}'?</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" @click="cerrarModalEliminar()">Cerrar</button>
                        <button type="button" class="btn btn-danger" @click="eliminarEvento(modalEliminar.id)"
        ...
        <div class="container" v-if="!loading">
            <div class="d-flex align-items-center mt-4">
                <router-link to="/evento/alta" class="text-center btn btn-success"><i
                        class="bi bi-plus-lg me-2"></i>Nuevo</router-link>

                <div class="alert alert-success mx-auto p-2" v-if="exitoEliminar">Evento '{{ modalEliminar.nombre
                ...
                <table class="table mx-auto text-center">
                    ...
                    <tbody>
                        <tr v-for=" evento  in  eventos " :key="evento.id">
                            ...
                            <td>{{ evento.fecha }}</td>
                            <td>{{ evento.lugar }}</td>
                            <td><button class="bi bi-eye btn" @click="verDetalles(evento.id)"></button>
                                <button class="bi bi-pencil btn" @click="editarEvento(evento.id)"></button>
                                <button class="bi bi-trash btn"
                                    @click="mostrarModalEliminar(evento.id, evento.nombre)"></button>
                            </td>
                        </tr>
                    </tbody>
                </table>
...
const verDetalles = (eventoId) => {
    router.push("/evento/" + eventoId);
};
const editarEvento = (eventoId) => {
    router.push("/evento/" + eventoId + "/editar");
}
</script>
...
export default {
    async mounted() {
        ...
        await eventoStore.listarEventos()
    ...
    methods: {
        async eliminarEvento(eventoId) {
            ...
            await this.eventoStore.eliminarEvento(eventoId)
                .catch(e => console.error(e))
                .then(data => {
                    if (data.status == 200) {
