  ...
  <DefaultLayout>
    <form @submit="editarRegistro">
      <div class="container d-flex flex-column mt-5" v-if="!loadingTitulares">
        <h2 class="text-center mb-4 center-content">Registro de Propiedad Industrial - Editar</h2>
        ...
        <div class="d-flex flex-column gap-3 flex-md-row justify-content-center">
          ...
          <div class="mb-3">
            ...
            <input v-model="registro.tipo" type="text" class="form-control" id="tipo" placeholder="Tipo" required>
          </div>
...
export default {
  ...
  methods: {
    editarRegistro: async function (e) {
      ...
      this.loading = true;
      await this.registroStore.editarRegPropIndustrial(this.$route.params.id, this.registro)
        .catch(e => console.error(e))
        .then(data => {
          this.loading = false;
          if (data.status == 200) {
            this.mensajeExito = true;
            setTimeout(() => this.$router.push({ name: 'listadoRegPropIndustrial' }), 5000)
