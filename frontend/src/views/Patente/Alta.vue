<template>
  <DefaultLayout>
    <form @submit="crearPatente">
      <div class="container d-flex flex-column mt-5" v-if="!loadingTitulares">
        <h2 class="text-center mb-4 center-content">Patentes - Alta</h2>

        <div class="d-flex flex-column  flex-md-row justify-content-center">
          <div class="mb-3 me-3">
            <label for="titulo" class="form-label fw-bold">Titulo</label>
            <input v-model="patente.titulo" type="text" class="form-control" id="titulo" placeholder="Nombre" required>
          </div>
          <div class="mb-3">
      ...
      <div class="d-flex justify-content-center  mb-5 mt-3">
        ...
        <button type="submit" class="btn btn-success" :disabled="loading">
          ...
          <i class="bi bi-plus me-1" :class="{ 'd-none': loading }"></i>Crear Patente
        </button>
      </div>
    </form>

    <div class="alert alert-success align-self-center" v-if="mensajeExito">Patente creada exitosamente.
...
export default {
  ...
  methods: {
    crearPatente: async function (e) {
      ...
      this.loading = true;
      await this.patenteStore.crearPatente(this.patente)
